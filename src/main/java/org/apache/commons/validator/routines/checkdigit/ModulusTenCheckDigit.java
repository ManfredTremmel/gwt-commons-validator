...
public final class ModulusTenCheckDigit extends ModulusCheckDigit {
     ...
     * CharacterGetNumericValue.getNumericValue(char) returns a non-negative number.
     *
     ...
     * @throws CheckDigitException if CharacterGetNumericValue.getNumericValue(char) returns a
     *             negative number
     */
    @Override
    protected int toInt(char character, int leftPos, int rightPos) throws CheckDigitException {
        int num = CharacterGetNumericValue.getNumericValue(character);
        if (num < 0) {
