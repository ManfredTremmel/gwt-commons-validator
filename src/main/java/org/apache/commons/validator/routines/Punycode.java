...
public class Punycode {
	 ...
	 */
	public static String encodeDomain(final String input) throws PunycodeException {
		if (input == null) {
			throw new PunycodeException("empty input not allowed");
		}
	...
	public static String encode(String input) throws PunycodeException {
		...
		for (int i = 0; i < input.length(); i++) {
			...
			int charAsInt = c;
			if ((charAsInt <= 0x9F && (charAsInt >= 0x7F || (charAsInt >>> 5 == 0)))
			  || charAsInt > 55296) {
     ...
     */
    private final static boolean isOnlyASCII(String input) {
        if (input == null) {
            return true;
        }
        for(int i=0; i < input.length(); i++) {
